...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            for i,corner in enumerate(self.corners):
                if (nextx,nexty)==corner: visited[i]=True
            newstate = ((nextx,nexty),tuple(visited))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    visited = state[1]
    heuristic_cost = 0 #distance to the nearest remaining food
    for i,corner in enumerate(corners):
        if not visited[i]: heuristic_cost = max(heuristic_cost,util.manhattanDistance(state[0],corner))
